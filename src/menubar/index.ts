...
interface MenuBarMenuWithElements extends MenuItem {
...
export class MenuBar extends Disposable {
  ...
  private overflowLayoutPasses: number = 0
  ...
  constructor(
    ...
    private currentOptions: MenuBarOptions,
  ...
  push(menu: Menu): void {
    ...
    topLevelMenus.forEach((menuBarMenu) => {
      ...
      if (this.isCompact) {
        ...
        const titleElement = $('.cet-menubar-menu-title', {
          role: 'none',
          'aria-hidden': true
        })
    ...
    // Recompute overflow after adding all top-level menus so initial render has the right state.
    this.updateOverflowAction()
  }
  ...
  createOverflowMenu(): void {
    ...
    const titleElement = $('.cet-menubar-menu-title.cet-toggle-more', {
      role: 'none',
      'aria-hidden': true
    })
  ...
  private updateOverflowAction(scheduleStabilizationPass = false): void {
    ...
    // The title is centered with position:absolute at 50% of the titlebar.
    // The menubar can use the space up to where the title starts, i.e. half
    // the titlebar width minus half the title width minus a small margin.
    const titleEl = this.container.parentElement?.querySelector(
      '.cet-title'
    ) as HTMLElement | null
    const titlebarWidth =
      this.container.parentElement?.offsetWidth ?? window.innerWidth
    const titleWidth = titleEl?.offsetWidth ?? 0
    const menuBarMargin = 20 // matches the CSS margin-right on .cet-menubar

    // Space from the start of the menubar container to just before the centered title
    const sizeAvailable =
      titlebarWidth / 2 - titleWidth / 2 - menuBarMargin - containerRect.left
    ...
    // Temporarily show overflow button to measure its width reliably
    const wasOverflowHidden =
      this.overflowMenu.buttonElement.style.display === 'none'
    if (wasOverflowHidden) {
      this.overflowMenu.buttonElement.style.display = ''
    }
    const overflowButtonWidth =
      this.overflowMenu.buttonElement.getBoundingClientRect().width ||
      this.overflowMenu.buttonElement.offsetWidth ||
      28
    if (wasOverflowHidden) {
      this.overflowMenu.buttonElement.style.display = 'none'
    }

    // Reset: show all menus so getBoundingClientRect() returns real widths.
    // This also fixes menus not reappearing when the window grows back.
    for (const menuBarMenu of showableMenus) {
      menuBarMenu.buttonElement.style.display = ''
    }

    for (const menuBarMenu of showableMenus) {
      if (!full) {
        const size = menuBarMenu.buttonElement.getBoundingClientRect().width
        const reserveOverflowButton =
          !this.isCompact && this.numMenusShown < showableMenus.length - 1
        const requiredSize =
          currentSize + size + (reserveOverflowButton ? overflowButtonWidth : 0)

        if (requiredSize > sizeAvailable) {
          full = true
        } else {
          currentSize += size
          this.numMenusShown++
      ...
      if (full) {
        menuBarMenu.buttonElement.style.display = 'none'
      }
    }

    // Overflow
    if (this.isCompact) {
      this.rebuildOverflowMenu(showableMenus, this.numMenusShown)

      this.overflowMenu.buttonElement.remove()
      if (this.numMenusShown < showableMenus.length) {
        this.container.insertBefore(
          this.overflowMenu.buttonElement,
  ...
  private updateLabels(
    ...
    if (this.options.enableMnemonics) {
      ...
      if (escMatch) {
        ...
        titleElement.append(
          strings.ltrim(
            replaceDoubleEscapes(cleanLabel.substring(0, escMatch.index)),
            ' '
          ),
          $('mnemonic', { 'aria-hidden': 'true' }, escMatch[3]),
          strings.rtrim(
            replaceDoubleEscapes(
              cleanLabel.substring(escMatch.index + escMatch[0].length)
            ),
            ' '
          )
        )
  ...
  private showMenu(menuIndex: number, selectFirst = true): void {
    ...
    if (!customMenu.submenu || !customMenu.buttonElement) {
      return
    }
