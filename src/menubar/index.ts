...
export class MenuBar extends Disposable {
  ...
  constructor(private container: HTMLElement, private currentOptions: MenuBarOptions, private options: IMenuBarOptions, private closeMenu = () => { }) {
    super()
  ...
  /*  private setupMenu() {
     ...
     topLevelMenus.forEach((menuItem) => {
       const menus: MenuBarMenu[] = asArray()
       const menuIndex = this.menuItems.length;
       const menuLabel = cleanMnemonic(menuItem.label)
  ...
  */
  ...
  private showMenu(menuIndex: number, selectFirst = true): void {
    ...
    const menuWidget = this._register(new CETMenu(menuHolder, this.currentOptions, menuOptions, this.closeMenu));
