...
import { BrowserWindow, IpcMainEvent, IpcMainInvokeEvent, Menu, MenuItem, TitleBarOverlay, ipcMain } from 'electron'
...
export default () => {
	...
	ipcMain.handle(IpcChannels.REQUEST_APPLICATION_MENU, async () => JSON.parse(JSON.stringify(
		Menu.getApplicationMenu(),
		(key: string, value: unknown) => (key !== 'commandsMap' && key !== 'menu') ? value : undefined)
	))

	// Handle window events
	ipcMain.on(IpcChannels.WINDOW_EVENT, (event: IpcMainEvent, eventName: WindowEventName) => {
	...
	// Handle menu events
	ipcMain.on(IpcChannels.MENU_EVENT, (event: IpcMainEvent, commandId: number) => {
	...
	ipcMain.on(IpcChannels.SET_MINIMUM_SIZE, (event: IpcMainEvent, width: number, height: number) => {
		const window = BrowserWindow.fromWebContents(event.sender)
		if (!window) return
	...
	ipcMain.handle(IpcChannels.REQUEST_MENU_ICON, async (event: IpcMainInvokeEvent, commandId: number) => {
		const item = getMenuItemByCommandId(commandId, Menu.getApplicationMenu())
	...
	ipcMain.handle(IpcChannels.GET_WINDOW_MAXIMIZED, (event: IpcMainInvokeEvent) => {
		const window = BrowserWindow.fromWebContents(event.sender)
		return window ? window.isMaximized() : false
	})

	ipcMain.handle(IpcChannels.UPDATE_WINDOW_CONTROLS, (event: IpcMainInvokeEvent, args: TitleBarOverlay) => {
		return updateWindowControls(event, args)
	})

	ipcMain.on(IpcChannels.UPDATE_WINDOW_CONTROLS, (event: IpcMainEvent, args: TitleBarOverlay) => {
...
function updateWindowControls(event: IpcMainEvent | IpcMainInvokeEvent, args: TitleBarOverlay): boolean {
	const window = BrowserWindow.fromWebContents(event.sender)
	try {
...
function getMenuItemByCommandId(commandId: number, menu: Electron.Menu | null): MenuItem | undefined {
